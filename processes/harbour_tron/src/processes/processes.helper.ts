...
class TronProcessHelper {
  ...
  public async check_our_wallet_address(address: string) {
    try {
      console.log("&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&: ", this.tron_wallet_address, address?.toUpperCase())
      let wallet_data: any = await this.getKeyValuePair(this.tron_wallet_address, address?.toUpperCase())
      console.log("🚀 ~ TronProcessHelper ~ check_our_wallet_address ~ wallet_data:", wallet_data)
