...
class EthProcessHelper {
  ...
  public async fetchWalletAddress() {
    try {
      const getaddressQuery = `SELECT wa.wallet_address,wa.user_id, wa.wallet_name FROM wallets as wa INNER JOIN coins as co ON co.coin_id=wa.coin_id  WHERE wa.wallet_address != '' AND wa.wallet_address IS NOT null AND co.coin_family=${config.COIN_FAMILY_ETH} GROUP BY wa.wallet_address`;
      const addresses: any = await database.db_read.query(getaddressQuery, {
        type: Sequelize.QueryTypes.SELECT,
        logging: console.log
      });
      console.log("🚀 ~ TronProcessHelper ~ fetchWalletAddress ~ addresses:", addresses)
      for (let address of addresses) {
        this.setKeyValuePair(config.ETH_WALLET_ADDRESS, address.wallet_address, JSON.stringify({ address: address.wallet_address, user_id: address.user_id, email: address.email, wallet_name: address.wallet_name }));
      }
    } catch (error: any) {
  ...
  public check_our_wallet_address = async (address: string) => {
    try {
      let wallet_data: any = await this.getKeyValuePair(config.ETH_WALLET_ADDRESS, address?.toUpperCase());
      if (wallet_data) {
