...
class PolController implements OnlyControllerInterface {
  ...
  public async getNonce(req: Request, res: Response): Promise<Response> {
    ...
    try {
      console.log("")
      var walletNonce = await PolHelper.getWalletNonce(wallet_address);
      ...
      console.error("error in Pol getting nonce", err);
      await commonHelper.save_error_logs("pol_getNonce", err.message);
  ...
  public async getEstimationGas(
    ...
    let lang: any = req.headers['content-language'] || 'en';
    try {
      console.log("req body pol getEstimationGas >>>", req.body)
      ...
      if (
        req.coininfo &&
        req.coininfo.coin_family !== config.STATIC_COIN_FAMILY.POL
      ) {
        return response.success(res, {
        ...
        console.log("req.coininfo", req.coininfo);
        
        if (is_token) {
          estimationRes =
            await PolHelper.getErc20TokenTransferGasEstimationCost(
              token_address,
              token_abi,
              from,
              to,
          ...
          estimationRes = await PolHelper.getEthGasLimit(from, to, lang);
          console.log("CCC",estimationRes )
        }
      ...
      await commonHelper.save_error_logs("pol_getEstimationGas", err.message);
      return response.error(res, {
  ...
  public async rawDataString(req: Request, res: Response): Promise<Response> {
    let lang: any = req.headers['content-language'] || 'en';
    try {
      console.log("req body pol rawDataString >>>", req.body)

      var { my_address, dest_address, amount, coin_type }: RawDataInterface =
        req.body;
      const web3: Web3 = new Web3(
        Web3.givenProvider || config.NODE.POL_RPC_URL + ""
      );
      let minABI: AbiItem[] = [
        // transfer
        {
      ...
      await commonHelper.save_error_logs("pol_rawDataString", err.message);
      return response.error(res, {
  ...
  public async send(req: Request, res: Response) {
    let lang: any = req.headers['content-language'] || 'en';
    try {
      console.log("req body pol send >>>", req.body)
      ...
      var gasInPOL = await PolHelper.convertWeiToEth(maxGas);
      gas_price = gasInPOL;
      var gasInPOLForErc = 0;
      // ETH only
      if (req.coininfo.is_token == false && req.coininfo.coin_symbol == "POL" || req.coininfo.coin_symbol == "pol") {
        var withdrawAmount;
        if (dapp_transaction == 1) {
          withdrawAmount = await bigNumberSafeMath('0', "+", gasInPOL);
          withdrawAmount = exponentialToDecimal(Number(withdrawAmount));
        } else {
          withdrawAmount = await bigNumberSafeMath(amount, "+", gasInPOL);
          withdrawAmount = exponentialToDecimal(Number(withdrawAmount));
        }
        let ethBalance = await polWeb3.get_coin_balance(from, true);
        console.log("🚀 ~ send ~ ethBalance:", ethBalance)
        console.log("🚀 ~ send ~ withdrawAmount:", withdrawAmount)
        ...
        var ethBalance = await polWeb3.get_coin_balance(from, true);
        var ERC20TokenBalance = await polWeb3.get_erc20_token_balance(
          contractAddress,
          ABI as AbiItem[],
          req.body.from
        );
        if (Number(ethBalance) < Number(gasInPOL)) {
          return response.error(res, {
            data: {
              status: false,
        ...
        if (Number(ethBalance) < Number(gasInPOL)) {
          return response.error(res, {
            data: {
              status: false,
              message: language[lang].ETH_GAS(gasInPOLForErc)
            },
          });
        }
      }
      await PolHelper.sendEthOrTokens(
        req,
        lang,
        tx_raw,
      ...
      console.error("Error in Polygon send API >>>>>", err)
      await commonHelper.save_error_logs("pol_send", err.message);
      return response.error(res, {
        data: {
          message: language[lang].CATCH_MSG
        },
  ...
  public async sendSwapTrnx(req: Request, res: Response) {
    let lang: any = req.headers['content-language'] || 'en';
    try {
      console.log("req body pol sendSwapTrnx >>>", req.body)
      ...
      console.error("Error in POLYGON sendSwapTrnx API >>>>>", err)
      await commonHelper.save_error_logs("pol_sendSwapTrnx", err.message);
      return response.error(res, {
        data: {
          message: language[lang].CATCH_MSG
        },
...
export const PolControllers = new PolController();
