...
import { address_book_queries, address_book_wallet_queries, device_token_queries, notification_queries, user_queries, wallet_queries } from '../../helpers/dbHelper/index'
...
class UserController implements OnlyControllerInterface {
  ...
  public async createWallet(req: Request, res: Response) {
    console.log("REQUEST: createWallet", req.body)
    ...
    try {
      ...
      // Updating balance and also adding new wallet in wallet table
  ...
  public async editAddressBook(req: Request, res: Response) {
    let lang: any = req.headers['content-language'] || 'en';
    try {
      console.log("req body user addAddressBook>>>", req.body)
      let user_id: number = req.userId;
      let address_book_id: number = Number(req.body.address_book_id) || 0;
      let address_book_wallet_id: number = Number(req.body.address_book_wallet_id) || 0;

      let { wallet_address, contact_name, address, coin_family, wallet_name,  } = req.body;


      if (address_book_id) {
        let wallet_name_exist = await Models.AddressBookModel.findOne({
          where: { 
            user_id: user_id,
            name: contact_name 
          },
      ...
      if (wallet_name_exist) {
        ...
        await Models.AddressBookModel.update({ name: contact_name }, { where: { id: address_book_id, user_id: user_id } });
        return response.success(res, {
          data: {
            code: GlblCode.SUCCESS,
            status: true,
            message: language[lang].ADDRESS_BOOK_EDITED
          }
        });
      } else if (address_book_wallet_id) {
        await Models.AddressBookWalletsModel.update({ name: wallet_name, wallet_address: wallet_address }, { where: { id: address_book_wallet_id } });
        return response.success(res, {
          data: {
            code: GlblCode.SUCCESS,
            status: true,
      ...
      console.error("Error in user > add_address_book.", err)
      await commonHelper.save_error_logs("users_edit_address_book", err.message);
      return response.error(res, {
        data: {
          code: GlblCode.ERROR_CODE,
          status: false,
