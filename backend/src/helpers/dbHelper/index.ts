...
class DbHelper {
  ...
  public async saveWithdrawTxDetails(
    ...
    console.log("🚀 ~ DbHelper ~ req:", req.userId)
    try {
      ...
      }else if (req.coininfo.coin_family == CoinFamily.POL) {
        user_data = await redisClient.getKeyValuePair(config.POL_WALLET_ADDRESS, req.body.to.toUpperCase())
      }
      user_data = JSON.parse(user_data);
      console.log("🚀 ~ DbHelper ~ user_data:", user_data)
      to_user_id = user_data ? user_data?.user_id : 0;
      console.log("🚀 ~ DbHelper ~ to_user_id:", to_user_id)
