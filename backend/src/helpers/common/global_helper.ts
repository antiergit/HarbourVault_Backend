...
import CMCServices from "../../services/coinmarket/service.cmc";
import { polWeb3 } from "./web3_pol_helper";
...
class Global_helper {
        ...
        try {
            ...
            switch (coin_family) {
                ...
                case config.STATIC_COIN_FAMILY.TRX:
                    ...
                    break;
                case config.STATIC_COIN_FAMILY.POL:
                    tokenDetails = await polWeb3.searchToken(token_address, lang);
                    break;  
            }
            return tokenDetails;
        } catch (err: any) {
        ...
        try {
            switch (coinFamily) {
                ...
                case config.STATIC_COIN_FAMILY.BTC:
                    return  utxobtc.validate_btc_address(walletAddress);
                case config.STATIC_COIN_FAMILY.TRX:
                    return await trxWeb3.validate_trx_address(walletAddress)
                case config.STATIC_COIN_FAMILY.POL:
                    return await polWeb3.validate_pol_address(walletAddress)
            }
        } catch (err: any) {
    ...
    public async get_coin_gicko_by_token_address(token_address: string) {
        try {
            const cmc =  new CMCServices()
            const response  :any = await cmc.getTokenInfo(token_address)
            if(response.logo && response.id) {
                return { image: response.logo , id: response.id, about  : response.about };
            }
            return null;
        } catch (err: any) {
